package kr.co.ict;
// SQL 관련요소는 java.sql.*을 임포트 하면 대부분이 다 로드 됩니다.
import java.sql.*;
...
public class JDBCselect {
	...
	public static void main(String[] args) {
		...
		// 뭐리문은 Statement 라믄 변수에 저장해 날려줍니다.
		...
		// 조회해서 쓰는 경우가 많습니다.
		// 맨처음 ResultSet은 -1번이라는 임시번호를 타겟으로 잡습니다.
		// 이 번호를 옮기기 위해 .next()를 호출하면 다음 번호로 넘어갑니다.
		//rs.next();
		// get자료형(인덱스 번호), 1부터) or get자료형(컬럼명)을쓰면
		// 해당컬럼의 자료를 반환합니다.
		//System.out.println(rs.getString("uname"));
		//System.out.println(rs.getString("uid"));
		//System.out.println(rs.getString("upw"));
		//System.out.println(rs.getString("umail"));
		
		// while문을 이용해서 전체 select구문의 전체 결과를
		// 콘솔에 찍어주세요.
